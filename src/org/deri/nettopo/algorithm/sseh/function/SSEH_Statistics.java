...
public class SSEH_Statistics {
	...
	public SSEH_Statistics() throws Exception {
		maxK = 10;
		seedNum = SEED_NUM;
		sensorNodeNum = 0;
		...
		logWriter = new PrintWriter("D:/CKNPlus_TPGF_Stat.log");
	...
	public void run(int k, int nodeNum) throws DuplicateCoordinateException {
		...
		for(int i=1;i<=totalLoop;i++){
			...
			SSEH_MAIN sseh = new SSEH_MAIN();
			sseh.setK(k);
//			sseh.runForStatistics();
			...
			SSEH_StatisticsMeta meta = new SSEH_StatisticsMeta(k,nodeNum,sleepNum,sleepRate,
		...
		SSEH_StatisticsMeta oneMeta = new SSEH_StatisticsMeta(k,nodeNum,totalAverateSleepNum,totalSleepRate,
	...
	public static void main(String[] args) throws Exception {
		SSEH_Statistics statistics = new SSEH_Statistics();
		String header = SSEH_StatisticsMeta.OUT_PUT_HEAD() + new SSEH_StatisticsMeta(EXTRA_FIELDS).extraInfoHeader();
		statistics.getLogWriter().println(SSEH_StatisticsMeta.NET_INFO_HEAD());
		statistics.getLogWriter().println(header);
		
		System.out.println(SSEH_StatisticsMeta.OUT_PUT_HEAD());
		...
		statistics.getLogWriter().print("\n\n\n-----------  end time: " + SSEH_StatisticsMeta.time()+"  -----------");
...
class SSEH_StatisticsMeta implements Serializable{
	private static int NET_WIDTH = SSEH_Statistics.getNET_WIDTH();
	private static int NET_HEIGHT = SSEH_Statistics.getNET_HEIGHT();
	...
	public SSEH_StatisticsMeta(String[] extraName){
		k = 0;
		sleepNodes = 0;
		totalSensorNodes = 0;
		sleepRate = sleepNodes  / totalSensorNodes;
	...
	public SSEH_StatisticsMeta(int k, int totalSensorNodes, double sleepNodes, double sleepRate, double averagePathNum, double averageOrHopNum, double averageOpHopNum){
		this.k = k;
	...
	public SSEH_StatisticsMeta(int k, int totalSensorNodes, double sleepNodes, double sleepRate, double averagePathNum, double averageOrHopNum, double averageOpHopNum, String[] extraName, double[] extraInfo){
		this.k = k;
		this.totalSensorNodes = totalSensorNodes;
		this.sleepNodes = sleepNodes;
		this.sleepRate = sleepRate;
