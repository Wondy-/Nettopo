...
public class WirelessSensorNetwork implements Serializable {
	...
	public void resetAllNodesColor() {
		...
		while(iter.hasNext()){
			...
			if(name.contains("Sensor")){
				...
				resetNodeColorByID(nodeID, NodeConfiguration.SolarNodeColorRGB);
			}
		}
	}
	...
	public int[] getSensorActiveNodes(){
		...
		while(iter.hasNext()){
			VNode node = iter.next();
			if(node.getClass().getSimpleName().contains("Sensor")||node.getClass().getSimpleName().contains("Solar"))
	...
	/**Get all sensor id by type string
	 * @param Type String
	 * @return All sensor id
	 */
	public int[] getAllSensorIDByType(String type){
		int[] allNodesId = this.getAllNodesID();
		LinkedList<Integer> array = new LinkedList<Integer>();
		for(int i=0;i<allNodesId.length;i++){
			if(getNodeByID(allNodesId[i]).getClass().getSimpleName().contains(type)){
	...
	public int[] getAllNormalSensorID(){
		int[] allNodesId = this.getAllNodesID();
		LinkedList<Integer> array = new LinkedList<Integer>();
		for(int i=0;i<allNodesId.length;i++){
	...
	public int[] getAllSensorNodesID(){
		...
		LinkedList<Integer> array = new LinkedList<Integer>();
		for(int i=0;i<allNodesId.length;i++){
			if(getNodeByID(allNodesId[i]).getClass().getSimpleName().contains("Sensor")||getNodeByID(allNodesId[i]).getClass().getSimpleName().contains("Solar")){
				array.add(allNodesId[i]);
			}
		}
		Integer[] result = array.toArray(new Integer[array.size()]);
